    ...
    static async parse({ downloader }: ParserOptions): Promise<ParserResult> {
        downloader.setOnKeyUpdated(async ({ keyUrls, explicitKeys, m3u8Url, saveEncryptionKey }) => {
            ...
            for (let i = 0; i <= keyUrls.length - 1; i++) {
                keys[CommonUtils.buildFullUrl(m3u8Url, keyUrls[i])] = explicitKeys[i] || "";
            }
            // download all keys
            let counter = 1;
            for (const url of Object.keys(keys)) {
                ...
                if (explicitKeys[counter - 1]) {
                    saveEncryptionKey(CommonUtils.buildFullUrl(m3u8Url, url), explicitKeys[counter - 1]);
                    continue;
                }
                ...
                while (retryCounter > 0) {
                    try {
                        ...
                        const hexKey = Array.from(new Uint8Array(response.data))
                            ...
                            .join("");
                        saveEncryptionKey(CommonUtils.buildFullUrl(m3u8Url, url), hexKey);
                        break;
                    } catch (e) {
                        if (retryCounter > 1) {
                            retryCounter--;
